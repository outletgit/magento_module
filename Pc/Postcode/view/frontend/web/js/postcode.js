 ...
 */
	...
	function postcodecheckout_initShipping(bReset)
	{
		...
		jQuery( "[name='shippingAddress.pc_postcode_postcode']" ).before("<strong class='custom-shipping-title'><label for='name'>Vul Hieronder je verzendadres in:</label></strong>");
		jQuery( ".pc_postcode strong" ).insertBefore( "[name='shippingAddress.pc_postcode_postcode']" );
	}
	...
	function postcodecheckout_disableFields(oElement)
	{
		...
		console.log('Toon magento velden, verberg de Postcode Checkout');

            if (registry.get(this.parentName + '.street'))
			{

                jQuery('.pc-postcode-hide').removeClass('pc-postcode-hide').addClass('field street admin__control-fields required pc-postcode-show iosc-whole').css("width", "100%");
                jQuery('.street').addClass('pc-postcode-show iosc-whole').css("width", "100%");
                registry.get(this.parentName + '.street').set('additionalClasses', 'field street admin__control-fields required iosc-whole');
                // registry.get(this.parentName + '.street').set('visible', true).set('labelVisible', false).set('disabled', false);
            }
            //jQuery('.field-note').css("display","none");
            ...
            if (registry.get(this.parentName + '.pc_postcode_fieldset.pc_postcode_address'))
            {
                registry.get(this.parentName + '.pc_postcode_fieldset.pc_postcode_address').set('visible', false);
            }
            if (registry.get(this.parentName + '.pc_postcode_fieldset.pc_postcode_city'))
            {
                registry.get(this.parentName + '.pc_postcode_fieldset.pc_postcode_city').set('visible', false);
            }
